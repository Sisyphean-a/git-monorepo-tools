...
- 后端把终端输出合并为批次再通过 `repo-terminal-output` 事件发送，关闭时先发送尾部内容；前端 `terminal-workspace` module 通过最小 `TerminalRuntime` interface 拥有会话 lifecycle、按 `sessionId` 的事件分发和按仓库的状态聚合，不能把一个终端的事件写入另一个终端。终端暂时不可见时可有界保留输出尾部；若因此丢弃较早积压，恢复显示前必须清空已显示历史并标记跳过，不能把不连续的旧历史和新尾部拼接。`RepoTerminalSurface` 只负责 xterm 显示与输入转发，不直接订阅运行时事件或调用会话绑定。
- 应用关闭时终止全部会话；Windows ConPTY 进程放入 kill-on-close Job，避免遗留子进程。

## 代码锚点

`internal/terminal/manager.go`、`internal/terminal/output_batcher.go`、`internal/terminal/host_windows.go`、`app.go`、`src/app/features/terminal/terminal-workspace.tsx`、`src/app/components/repo-terminal-surface.tsx`。
