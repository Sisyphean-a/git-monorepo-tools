...
- `TerminalProtocolObserver` 只观察交给 xterm 的输出副本，绝不筛除、改写或路由该输出；它在 xterm `onWriteParsed` 后读取公开的受控粘贴 mode，并在 `onData` 观察键盘协商响应。它还从 OSC 0 标题识别默认 Pi 标题，且仅与已确认的 Pi 输入协议组合后决定粘贴运输路径。终端诊断弹窗因此能区分“Pi 已请求”“输出待 xterm 处理”“xterm 已确认”和“已收到协商响应”。
