...
- Windows 粘贴先用原生剪贴板格式检查图片；仅检测到图片时才启动隐藏的 PowerShell，将它落为 `%TEMP%/pi-clipboard-*.png` 并按输入队列写入终端。没有图片时不启动子进程，直接保留 xterm 的文本粘贴转换。例外是已确认的 Pi 会话：`TerminalProtocolObserver` 必须同时观测到 Pi 的 OSC 0 标题（`π` 或 `π - <非空名称>`）、已请求且已由 xterm 确认的 bracketed paste、以及已由 xterm 协商确认的增强键盘；这时 `Ctrl+V`、`Alt+V` 和右键粘贴都绕过应用的剪贴板读取与 xterm bracketed-paste 包装，按输入队列写入 Pi Windows 默认 `Alt+V` 的 modifyOtherKeys 序列 `ESC[27;3;118~`，让 Pi 自己读取文字或图片剪贴板。Pi 其后有普通输出正在等待 xterm 异步解析时，此资格仍有效；键盘协商仍在等待响应时必须走默认路径，Pi 关闭 bracketed paste、改变标题或会话结束后也必须回到默认路径。`repo-terminal-shortcuts.ts` 的 `windowsTerminalShortcutRules` 是 Pi Windows 输入归一的唯一配置位置：只有完整匹配的 `Shift+Enter`、`Ctrl+J`、`Ctrl+Backspace`、`Ctrl+V`、`Alt+V` 和有选区时的 `Ctrl+C` 被应用接管；其余按键及额外修饰键（包括 `Ctrl+←/→`）均保持 xterm 透传。`Shift+Enter` 与 `Ctrl+J` 都发送原始 LF，作为 Pi 跨终端稳定识别的新行输入；`Ctrl+Backspace` 发送 `Ctrl+W`，匹配 PowerShell PSReadLine 的按词删除绑定。
...
- `TerminalProtocolObserver` 只观察交给 xterm 的输出副本，绝不筛除、改写或路由该输出；它在 xterm `onWriteParsed` 后读取公开的受控粘贴 mode，并在 `onData` 观察键盘协商响应。为处理跨块控制序列保留的输出尾部只能补全未完成的序列，不能重新应用此前已完整匹配的协议状态。它还从 OSC 0 标题识别默认 Pi 标题，且仅与已确认的 Pi 输入协议组合后决定粘贴运输路径。终端诊断弹窗因此能区分“Pi 已请求”“输出待 xterm 处理”“xterm 已确认”和“已收到协商响应”。
- Windows ConPTY 会在 Pi 0.84.1 全屏启动时丢弃 DEC 鼠标开启序列，却保留备用屏幕、受控粘贴和增强键盘序列；在受控粘贴已开启时，它还可能在 `ConPTY.Write` 与 Node stdin 之间吃掉输入的 `ESC[200~`/`ESC[201~` 边界，尽管前端和后端各自记录的是完整单次写入。`RepoTerminalSurface` 只有在观察到“已进入备用屏幕、已请求受控粘贴、增强键盘已请求或协商、且未收到鼠标序列”这一完整 Pi 指纹时，才把同一组鼠标模式写入本地 xterm；原始后端输出不改写，也不伪造浏览器滚轮。Pi 退出备用屏幕或终端进程退出时必须关闭这组本地模式，使 xterm 原生把滚轮编码为 SGR 鼠标输入且不影响其他终端程序。
