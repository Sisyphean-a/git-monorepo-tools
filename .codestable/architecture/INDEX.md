...
- 启用自动扫描时，当前选中的项目立即执行本地 Git 状态刷新并每 2 秒重试，其余已发现项目每 10 秒刷新；这些高频刷新不访问远端，原自动扫描间隔继续负责远端刷新。后台结果若跨越了交互操作则不得回写。仓库更新在 `mergeRepoSnapshotUpdate` 按来源合并：后台状态轮询更新 Git 状态字段，并从同一次 porcelain v2 status 读取 `HEAD`：`HEAD` 未变时保留历史阅读数据与版本，`HEAD` 变化时重新加载历史；交互更新递增前端单调阅读版本，使提交、拉取、推送等操作重新加载历史。两类更新都复用语义未变的 `FileChange` 数组与对象。周期快照可以更新变更列表与计数，但不得仅因扫描时间、`RepoDetail` 或等价 `FileChange` 对象替换而重建已展开的文件差异或取消进行中的读取；只有仓库、请求设置或文件的可观察变更摘要变化时才重新读取差异。展开差异时普通路径只执行一次定向 `git diff`，不保留结果缓存；每次展开都重新读取。历史页的提交列表、选择和详情同属阅读状态：周期快照会省略历史数据，不得因此清空或重载当前历史；只有仓库 ID 真正变化时才重置并加载。
