...
type WailsBindings = {
  ...
  RunBatch: (operation: 'pull' | 'push', request: SnapshotRequest) => Promise<BatchResponse>;
...
export async function fetchRepoHistory({ repoId, offset, limit, settings }: RepoHistoryRequest) {
  return getWailsBindings().GetRepoHistory(repoId, buildSnapshotRequest(settings), offset, limit);
}

export async function fetchCommitDetail({ repoId, hash, settings }: CommitDetailRequest) {
...
export async function fetchFileDiff({ repoId, filePath, previousPath, status, staged, untracked, commitHash, parentHash, settings, target }: FileDiffRequest) {
...
export async function runRepoCommand({ repoPath, command, streamId, settings }: RepoCommandRequest) {
...
export async function readClipboardImagePath() {
  const binding = getWailsBindings().ReadClipboardImagePath;
  if (typeof binding !== 'function') {
    throw new Error('Wails 图片剪贴板绑定不可用');
  }
