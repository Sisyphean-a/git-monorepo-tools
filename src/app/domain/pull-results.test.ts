...
function result(overrides: Partial<PullResult> & Pick<PullResult, 'id' | 'result'>): PullResult {
  return {
    id: overrides.id,
    name: overrides.id,
...
test('sortPullResults puts changed and exceptional repos before no-op repos', () => {
  ...
  assert.deepEqual(
    ...
    ['failed', 'pulled', 'skipped', 'uptodate-a', 'uptodate-b'],
  );
});
...
test('sortPullResults keeps same-priority repos stable except for larger commit counts first', () => {
  ...
  assert.deepEqual(
    ordered.map(item => item.id),
    ['pulled-large', 'pulled-small', 'skipped-a', 'skipped-b'],
  );
});
