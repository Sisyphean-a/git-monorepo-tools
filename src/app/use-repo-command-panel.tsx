...
import type { AppSettings, BuiltInCommandAction, CommandCombo, CustomCommandButton, RepoCommandResult, RepoDetail, RepoMutationAction } from './types';
...
export function useRepoCommandPanel({
  ...
  const triggerBusyAction = (actionKey: string, handler: (isActive: () => boolean) => Promise<void>) => {
    void runBusyAction(actionKey, handler).catch(() => {});
  };
  ...
  const executeBuiltInAction = async (
    ...
    if (action === 'generate') {
      try {
        ...
        const nextMessage = await generateCommitMessage(repo.id, settings);
  ...
  const runCustomCommand = (command: CustomCommandButton) => {
    triggerBusyAction(`custom:${command.id}`, async isActive => {
      ...
      const stopListening = EventsOn('repo-command-output', payload => {
        if (!payload || payload.streamId !== streamId) return;
        session.write(typeof payload.chunk === 'string' ? payload.chunk : '');
      });
