...
Windows 的 Pi 专属规则只在 `repo-terminal-shortcuts.ts` 的 `windowsTerminalShortcutRules` 定义：`Shift+Enter` 与 `Ctrl+J` 都写入原始 LF，`Ctrl+Backspace` 写入 `Ctrl+W`；`Ctrl+V` 与 `Alt+V` 走受控剪贴板路径；仅在存在选区时把 `Ctrl+C` 解释为复制。图片剪贴板优先传入临时 PNG 路径，文本仍由 xterm 的 `paste` 编码，二者都进入同一按会话串行写入队列。
